...
static void sigusrFunc(int sig)
{
    mqFlag = 1;
    return;
}
...
int main(int argc, char const *argv[])
{
    ...
    signal(SIGUSR1, sigusrFunc); // 注册SIGUSR1信号处理函数
    ...
    for (;;)
    {
        // 将当前信号屏蔽字添加newmask，并将原来的信号屏蔽字赋予oldmask
        sigprocmask(SIG_BLOCK, &newmask, &oldmask);
        while (0 == mqFlag)
        {
            // 将进程信号屏蔽字改为zeromask，并在捕捉到一个信号或发生了一个会终止该进程的信号之前，该进程被挂起
            sigsuspend(&zeromask);
        }
        mqFlag = 0;
